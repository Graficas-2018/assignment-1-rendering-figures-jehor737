...
function createRhombus(gl)
{
    ...
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    //Location of the vertices for the rhombus
    var verts = [
      0.0,0.85,0.0, //top
      -0.5,0.333,0.0, //left
      0.5,0.333,0.0, //right
...
function createSphere(gl, radius)
{
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    var verts = [];
    //Use the properties of unit circle for (x,y) = (cos(θ), sin(θ))
    verts.push(0,0,0);
    for (var i = 35; i <= 310; i++){
      verts.push(
        radius * Math.cos(i*Math.PI/180), radius * Math.sin(i*Math.PI/180), 0 //convert to radians and multiply by the radius for the size and add to the vertices array
      );
    }
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    // The resulting object contains the vertexbuffer, the size of the vertex structure (3 floats, x, y, z), the number of vertices to be drawn, the the primitive to draw.
